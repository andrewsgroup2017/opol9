...
app.set('gmaps', 'AIzaSyADb9NP2KUL4SAw4KhGY2eda3nIzhIz5_g' || undefined);
...
app.use(function (req, res, next) {
  ...
  if (!req.gf.titleString) {
    req.gf.titleString = "Locative";
  }
  if (!req.gf.utils) {
    req.gf.utils = utils;
  }
  if (!req.gf.config) {
    req.gf.config = config;
  }
  if (!req.gf.async) {
    req.gf.async = async;
  }
  if (!req.gf.socketClients) {
    req.gf.socketClients = socketClients;
  }
  if (!req.gf.analytics) {
    req.gf.analytics = new analytics();
  }
  next();
});
