...
var getDirection = function(callFrom, callTo) {
	...
	if (
		callFrom.split('@')[1] == 'andrewsgroup.onsip.com' ||
		callFrom.split('@')[0] == 'sip:19035663081' ||
		callFrom.split('@')[0] == 'sip:19035663068'
	) {
		return 'out'
	} else {
		return 'in'
...
var getNumber = function(callFrom, callTo, direction) {
	...
	if (direction == 'out') {
		return callTo.substring(callTo.lastIndexOf('sip:') + 4, callTo.lastIndexOf('@'))
	} else {
		// console.log(callFrom)
		return callFrom.substring(callFrom.lastIndexOf('sip:') + 4, callFrom.lastIndexOf('@'))
	}
}
...
async function getAccount(_phoneNumber) {
	...
	return new Promise(function(resolve, reject) {
		...
		unirest
			.get(`https://api.servicemonster.net/v1/accounts?wField=phone1&wValue=${phoneNumber}&wOperator=like`)
			.headers({
			...
			.end(function(response) {
				if (response.body.items.length < 1) {
					unirest
						.get(
							`https://api.servicemonster.net/v1/accounts?wField=phone2&wValue=${phoneNumber}&wOperator=like`
						)
						.headers({
							Accept: 'application/json',
							'Content-Type': 'application/json',
						...
						.end(function(response) {
							...
							if (response.body.items.length < 1) {
								unirest
									.get(
										`https://api.servicemonster.net/v1/accounts?wField=phone3&wValue=${phoneNumber}&wOperator=like`
									)
									.headers({
										Accept: 'application/json',
										'Content-Type': 'application/json',
